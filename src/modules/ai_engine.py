...
def calculate_ai_metrics(df_assets, df_maint, df_lic, df_staff=None):
    ...
    # -------------------------------------------------
    # THỜI GIAN (FIX TZ-AWARE & INDENTATION)
    # -------------------------------------------------
    # Sử dụng Naive Timestamp để tránh lỗi trừ khác múi giờ
    ...
    if 'created_at' in df_ai_base.columns:
        df_ai_base["created_at"] = (
            ...
            .dt.tz_localize(None) # Tước bỏ múi giờ để đồng bộ với 'now'
            .fillna(now)
        )
    else:
    ...
    if df_maint is not None and not df_maint.empty:
        df_maint['date'] = pd.to_datetime(df_maint['date'], errors='coerce').dt.tz_localize(None)
        mtbf_days = df_maint.sort_values(['asset_tag', 'date']).groupby('asset_tag')['date'].diff().dt.days.mean()
        ai_metrics["mtbf"] = f"{round(mtbf_days, 1)} ngày" if pd.notna(mtbf_days) else "N/A"

        if 'repair_time_hours' in df_maint.columns:
            ai_metrics["mttr"] = f"{round(df_maint['repair_time_hours'].mean(), 1)}h"

    # -------------------------------------------------
    # LICENSE
    # -------------------------------------------------
    if df_lic is not None and not df_lic.empty:
        df_lic['expiry_date'] = pd.to_datetime(df_lic['expiry_date'], errors='coerce').dt.tz_localize(None)
