...
try:
    ...
    from src.modules import dashboard, inventory, servers, licenses, vault, auth # Thêm module auth
...
# 4. KHỞI TẠO KẾT NỐI & TRẠNG THÁI
@st.cache_resource
def init_connection():
    try:
        return get_supabase()
    except Exception:
        st.error("⚠️ Không thể kết nối tới cơ sở dữ liệu.")
        st.stop()
